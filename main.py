...
def download(s3, s3p: str, output_file: str) -> str:
    def s3_download(s3, bucket_name: str, key: str, local_fn: str):
        ...
        assert os.path.exists(
            local_fn
        ), f"the local file {local_fn} should have been downloaded"
        return local_fn
    ...
    assert os.path.exists(
        the_directory
    ), f"the directory {the_directory} should exist but does not."
    ...
    try:
        ...
        utils.log(
            "something has gone horribly awry when trying to download the S3 file: %s"
            % e
        )
...
def handle_podcast_episode_creation_request(
    ...
    s3.meta.client.upload_file(
        output_podcast_audio_local_fn, bucket, f"{folder}/{file}"
    )
        ...
        while retry_count < max_retries:
            try:
                ...
                def build_rmq_uri():
                    ...
                    rmq_address = (
                        f"rmq://{rmq_username}:{rmq_pw}@{rmq_host}/{rmq_vhost}"
                    )
                    if DEBUG:
