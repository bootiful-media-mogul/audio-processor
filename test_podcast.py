...
def generate_sample_audio_files(say_output_file_dirname: str, parts: typing.List[str]):
    ...
    for to_say in parts:
        ...
        assert os.path.isdir(
            say_output_file_dirname
        ), f"the directory in which you want to write {say_output_file_name} does not exist"
...
class TestCreatingPodcastAudio(TestCase):
    ...
    def test_creating_podcast_audio(self):
        ...
        if not os.path.exists(sample_audio_folder) or len(
            os.listdir(sample_audio_folder)
        ) < len(numbers):
            print("creating sample audio files")
            samples = generate_sample_audio_files(sample_audio_folder, to_say)
        else:
            samples = [
                os.path.join(sample_audio_folder, sample)
                for sample in os.listdir(sample_audio_folder)
                if sample.endswith(".aiff")
            ]
            # now we have the files, let's turn 'em into a podcast

        samples.sort()
        segments = [
            podcast.Segment(audio_file=sample, ext="aiff") for sample in samples
        ]
        output_podcast = podcast.create_podcast(
            segments, os.path.join(sample_audio_folder, "output.wav"), "wav"
        )
        assert os.path.exists(
            output_podcast
        ), f"the podcast file {output_podcast} was not successfully generated"
